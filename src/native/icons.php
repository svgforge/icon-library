 ...
 * is registered as an icon in the "sf-icon-manager" collection. The same sprite
 ...
 *
 * Registration is lazy and gated by the "sfim_native" setting
 ...
 */
const SFIM_ICONS_OPTION = 'sfim_icons';
 ...
 */
const SFIM_NATIVE_OPTION = 'sfim_native';
 ...
 */
function sfim_native_setting(): string
{
    $value = get_option(SFIM_NATIVE_OPTION, 'off');
 ...
 */
function sfim_icon_slug(string $id): string
{
 ...
 *
 * Source order mirrors sfim_sprite_url(): filter, upload, fallback.
 ...
 */
function sfim_sprite_source_path(): string
{
    return sfim_current_sprite()['path'];
}
 ...
 * @return string
 */
function sfim_sprite_source_signature(): string
{
    $sprite = sfim_current_sprite();
    $extra  = '';
 ...
 */
function sfim_sprite_icons(): array
{
    $signature = sfim_sprite_source_signature();
    $cached    = get_option(SFIM_ICONS_OPTION, null);

    if (is_array($cached) && ($cached['signature'] ?? '') === $signature && is_array($cached['icons'] ?? null)) {
        return $cached['icons'];
    }

    $icons = sfim_parse_sprite_icons(sfim_sprite_content());

    update_option(SFIM_ICONS_OPTION, ['signature' => $signature, 'icons' => $icons], false);
 ...
 * @return void
 */
function sfim_invalidate_native_icons(): void
{
    $GLOBALS['sfim_native_registered'] = false;
    delete_option(SFIM_ICONS_OPTION);
}
 ...
 * @return array[] List of ['name' => string, 'label' => string, 'content' => string].
 */
function sfim_parse_sprite_icons(mixed $svg): array
{
    $icons = [];
    ...
    foreach ($symbols as $symbol) {
        ...
        $slug = sfim_icon_slug($id);

        if ($slug === '') {
            continue;
        }

        $name = 'sf-icon-manager/' . $slug;
        ...
        foreach ($shapes as $shape) {
            $inner .= $document->saveXML(sfim_icon_shape($shape));
        }
 ...
 *
 * Unlike sfim_sprite_icons(), this enumerates the raw sprite symbols
 ...
 */
function sfim_icon_shape(DOMElement $shape): DOMElement
{
 ...
 * "Icon is already registered" warning. Use sfim_ensure_native_icons()
 * as the public entry point.
 *
 * @return void
 */
function sfim_register_native_icons(): void
{
    if (! empty($GLOBALS['sfim_native_registered'])) {
        return;
    }

    if (sfim_native_setting() !== 'on') {
        return;
    }

    if (! apply_filters('sfim_register_native_icons', true)) {
        return;
    }
    ...
    if ($icons === []) {
        $GLOBALS['sfim_native_registered'] = true;
 ...
 * @return void
 */
function sfim_ensure_native_icons(): void
{
 ...
 */
function sfim_ensure_on_icon_block(array $parsed_block): array
{
...
add_action('rest_api_init', 'sfim_ensure_native_icons');
add_filter('render_block_data', 'sfim_ensure_on_icon_block');
 ...
 */
function sfim_deregister_native_icon_block($block_name = 'core/icon'): void
{
    ...
    if (sfim_native_setting() !== 'no_block' || ! class_exists('WP_Block_Type_Registry')) {
        return;
    }
...
add_action('init', 'sfim_deregister_native_icon_block', PHP_INT_MAX);
add_action('rest_api_init', 'sfim_deregister_native_icon_block', PHP_INT_MAX);
add_action('admin_enqueue_scripts', 'sfim_deregister_native_icon_block', PHP_INT_MAX);
add_action('enqueue_block_editor_assets', 'sfim_deregister_native_icon_block', PHP_INT_MAX);
 ...
 */
function sfim_deny_native_icon_block_types(mixed $allowed): bool|array|null
{
    if (sfim_native_setting() !== 'no_block') {
...
add_filter('allowed_block_types_all', 'sfim_deny_native_icon_block_types');
 ...
 * @return string
 */
function sfim_strip_native_icon_block(string $block_content, array $block): string
{
    if (sfim_native_setting() !== 'no_block') {
...
add_filter('render_block', 'sfim_strip_native_icon_block', PHP_INT_MAX, 2);
 ...
 */
function sfim_native_unregister_block_editor_assets(): void
{
    if (sfim_native_setting() !== 'no_block') {
        return;
    }
...
add_action('enqueue_block_editor_assets', 'sfim_native_unregister_block_editor_assets');
